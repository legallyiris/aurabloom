...
class Logger {
  ...
  constructor(scope: string | string[] = [], options: LogOptions = {}) {
    ...
    this.options = {
      timestamp: true,
      colourize: true,
      scopecolour: this.getScopecolour(this.scope.join("/")),
      ...options,
    };
  ...
  private formatMessage(level: LogLevel, message: string): string {
    ...
    const levelcolour = this.getLevelcolour(level);
    const colourStart = this.options.colourize ? levelcolour : "";
    const colourEnd = this.options.colourize ? COLOURS.RESET : "";

    const formattedMessage = `${timestamp}${colourStart}[${levelString}]${colourEnd} ${this.options.colourize && this.options.scopecolour ? this.options.scopecolour : ""}${scopeString}${COLOURS.RESET}${message}`;
...
const singleton = new Logger("global", {
  timestamp: true,
  colourize: true,
  scopecolour: COLOURS.FG_MAGENTA,
});
